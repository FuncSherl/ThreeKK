...
class person(object):
    ...
    def add_box(self, info_list, withleft=True, withright=True):
        mlen=max([len(x.encode()) for x in info_list])+int(withright)+int(withleft)
        self.mlen=mlen
        for i in range(len(info_list)):
            info_list[i]=''.join( [  ('|' if withleft else '')  ,info_list[i], (' ' if withright else '')*(mlen-2-len(info_list[i].encode())), \
                                   '|' if withright else ''] )
    ...
    def get_hero_describe(self):
        ...
        mlen=max([len(x.encode()) for x in ret])
    ...
    def form_other_person(self):
        return ['INDEX:%d'%self.myindex,\
                'HERO:'+self.name_chinese+'  H:' +'*'*self.health, \
                'ARMERS:'+','.join( [Cards.class_list[x[0]].name_chinese for x in self.armers]),\
              'SHIELDS:'+','.join( [Cards.class_list[x[0]].name_chinese for x in self.shields]), \
              'HORSE+1:'+','.join( [Cards.class_list[x[0]].name_chinese for x in self.horse_plus]),\
              'HORSE-1:'+','.join( [Cards.class_list[x[0]].name_chinese for x in self.horse_minus]),  'CARDS:'+'N'*len(self.cards)]
